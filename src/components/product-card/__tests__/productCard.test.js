...
describe("Testing Product Card component", () => {
  test("Should add the product item when the product card button is clicked", () => {
    // Used async because (fireEvent) method is asynchronus
    const mockItem = {
      id: 1,
      imageUrl: "test",
      name: "Item A",
    ...
    const { store } = renderWithProviders(<ProductCard />, {
      // we can access the store by destructuring it from (renderWithProviders)
      preloadedState: {
        cart: {
          value: [],
        },
